...
class PaymentController extends Controller
{
    ...
    { 
        if($data->status==0)
        {
            ...
            if($user->user->refer!=0) 
            {
                ...
                $msg =  'Referal Commision of from '. $refer->user->name ;
                send_email($refer->user->email, $refer->user->username, 'Referal Commision', $msg);
                send_sms($refer->user->mobile, $msg);
            } 

            $msg =  'Purchase and Payment Successfull';
            send_email($user->user->email, $user->user->username, 'Purchase Successfull', $msg);
            send_sms($user->user->mobile, $msg);
        }
    ...
    public function purchaseConfirm()
    {
        ...
        if(is_null($data))
        {
            return redirect()->route('home')->with('alert', 'Invalid Sell Request');            
        }
        if ($data->status != 0) 
        {
            return redirect()->route('home')->with('alert', 'Invalid Sell Request');
        ...
        elseif ($data->gateway_id == 501) 
        {
            ...
            $bitcoin['amount'] = $SellData->btc_amo;
            $bitcoin['sendto'] = $SellData->btc_wallet;

            $var = "bitcoin:".$SellData->btc_wallet."?amount=".$SellData->btc_amo;
        ...
        elseif($data->gateway_id == 506)
        {
            ...
            if($data->btc_amo==0 ||$data->btc_wallet=="")
            {
                ...
                {
                    return back()->with('alert', 'Unable to Process');
                }
        ...
        elseif($data->gateway_id == 513)
		{
			...
			$form = $CP->createPayment('Purchase', 'BTC',  $bcoin, $ntrc, $callbackUrl);
            $pt = $method->name;
    ...
    {
        $SellData = Sell::where('status', 0)->where('gateway_id', 502)->where('try','<=',100)->get();
    ...
    public function blockIpnLite(Request $request)
    {
        ...
        $SellData = Sell::where('status', 0)->where('gateway_id', 503)->where('try','<=',100)->get();
    ...
    {
        $SellData = Sell::where('status', 0)->where('gateway_id', 504)->where('try','<=',100)->get();
    ...
    {
        $data = Sell::where('trx',$_POST['order_id'])->orderBy('id', 'DESC')->first();
